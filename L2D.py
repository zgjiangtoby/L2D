import torch, os, sys, re, ordered_set, math
...
class Finetune_XLM():
      ...
      def _forward(self, train_set, val_set):
            ...
            if self.task_name in ['mnli', 'qnli']:
                  train_loader = DataLoader(train_set, batch_size=16, shuffle=True, collate_fn=self._collate_fn)
                  val_loader = DataLoader(val_set, batch_size=16, shuffle=False, collate_fn=self._collate_fn)
            else:
...
class L2DRetriever(SimilarityRetriever):
      ...
      def retrieve(self, test_text, k=30, n=8):
...
class LLMEvaluator:
      def __init__(self, model_path, custom_id2label, max_new_tokens=50, gpu=1 ):
            
            self.gpu = gpu
            print(self.gpu)
            self.devices = [torch.device(f"cuda:{i}") for i in range(self.gpu+1)] 
            print(self.devices)
...
def inferencer(test_text, retirever, task_name, method, output_dir):
      test_text = test_text
      all_responses = []
      ...
      for text in tqdm(test_texts, desc="Generating Responses"):
            ...
            ice, input_text = retirever.retrieve(text)
...
if __name__ == "__main__":
      ...
      if args.method == "L2D":
            ...
            classifier._forward(f_train, f_val)
            cls_model = classifier.classifier

            retriever = L2DRetriever(train_dataset, args.retriever_model, classifier, id2label=custom_id2label, args=args)

   
      test_texts = [test_dataset[i][0] for i in range(len(test_dataset))]
      gold_labels = [test_dataset[i][1] for i in range(len(test_dataset))]

      llm = LLMEvaluator(args.llm_model, custom_id2label, gpu=args.gpu) 
      evaluator = LLMEvaluation(custom_id2label, task_type='classification')

      all_responses = inferencer(test_text=test_texts, retirever=retriever, task_name=args.task_name, method = args.method, output_dir = args.output_dir)
      ...
      with open(args.output_dir + 'result_new_{}_{}.txt'.format(args.alpha, args.method), 'w', encoding='utf-8') as f:
